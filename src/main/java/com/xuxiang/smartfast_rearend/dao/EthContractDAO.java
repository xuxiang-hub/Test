...
//使用jpa无需手动构建SQL语句
public interface EthContractDAO extends JpaRepository<EthContracts, Integer> {

    // 查询最新的contract对象，即id最大的contract对象
    @Query("SELECT c FROM EthContracts c WHERE c.id = (SELECT MAX(c2.id) FROM EthContracts c2)")
    EthContracts findLatestContract();
}
