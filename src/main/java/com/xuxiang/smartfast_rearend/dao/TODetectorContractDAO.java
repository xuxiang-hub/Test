...
//使用jpa无需手动构建SQL语句
public interface TODetectorContractDAO extends JpaRepository<TODetectorContracts, Integer> {

    // 查询最新的contract对象，即id最大的contract对象
    @Query("SELECT c FROM TODetectorContracts c WHERE c.id = (SELECT MAX(c2.id) FROM TODetectorContracts c2)")
    TODetectorContracts findLatestContract();
